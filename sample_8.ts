	...
	class DemoIconsWorld extends BasicView {
		...
		private createParticleCloud() {
			...
			var material = new THREE.PointCloudMaterial({
				size: 30, color: 0x444444, blending: THREE.AdditiveBlending,
		...
		private createLogo():void {
			...
			var text1 = new createjs.Text(this.WORD_LIST[this._wordIndex], "42px " + FONT_NAME, "#FFF");
			this._wordIndex++;
			...
			if (Math.random() < 0.3) {
				...
				timeline.to(this.camera.position, 15.0, {x: 0, y: 0, z: 5000, ease: Quart.easeInOut}, 0);
			}
			...
			if (Math.random() < 0.3) {
				...
				TweenMax.to(timeline, 4.0, {timeScale: 2.0, ease: Cubic.easeIn, delay: 5.0});
			} else {
